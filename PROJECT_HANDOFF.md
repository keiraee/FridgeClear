...
- 2026-08 界面第二轮打磨：详情收藏/匹配、冰箱编辑、确认弹窗、登录回跳等（详见 ROADMAP 阶段 F0）
...
| D | AI 备餐异步化 | ✅ |
| E | 工程化与文档 | ✅ |
| F | 界面体验第二轮 | 🔄 按需（见 `docs/ROADMAP.md` 阶段 F） |
...
### 界面体验（见 `docs/ROADMAP.md` 阶段 F）

- **F1 待办：** 详情库存对照、错误状态卡片、redirect 补全、冰箱手机按钮布局、备餐队列清空确认等
- **F2 待办：** 筛选样式统一、列表动态标题、首页过期提示、详情 sticky CTA、管理端对齐主站等
- **F3 可暂缓：** 列表匹配率、按天折叠、Footer 链接等
